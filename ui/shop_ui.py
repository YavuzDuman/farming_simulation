...
# Animal unlock levels
ANIMAL_UNLOCK_LEVELS = {
    ItemType.CHICKEN: 5,
    ItemType.COW: 8,
}
...
class ShopAnimal:
    """Represents an animal that can be bought in the shop"""
...
class ShopUI:
    ...
    def __init__(self):
        ...
        self.current_category = "seeds"  # "seeds", "tools", or "animals"
        ...
        # Define animals available for purchase
        self.animal_items: List[ShopAnimal] = [
            ShopAnimal("Chicken", ItemType.CHICKEN, 50, "A cute chicken that walks around your farm", unlock_level=5),
            ShopAnimal("Cow", ItemType.COW, 100, "A friendly cow that roams your farm", unlock_level=8),
        ]
        ...
        # Category buttons
        self.seeds_cat_rect = pygame.Rect(self.x + 20, self.y + 95, 80, 30)
        self.tools_cat_rect = pygame.Rect(self.x + 105, self.y + 95, 80, 30)
        self.animals_cat_rect = pygame.Rect(self.x + 190, self.y + 95, 80, 30)
        ...
        self.animal_buttons: List[Tuple[pygame.Rect, ShopAnimal]] = []
        ...
        self.on_buy_animal: Optional[Callable[[ShopAnimal], bool]] = None
    ...
    def handle_event(self, event: pygame.event.Event) -> Optional[str]:
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            ...
            if self.current_tab == "buy":
                ...
                if self.animals_cat_rect.collidepoint(mouse_pos):
            ...
            if self.current_tab == "buy":
                ...
                # Check animal items
                for rect, animal in self.animal_buttons:
                    if rect.collidepoint(mouse_pos):
                        # Try to buy the animal
                        if self.on_buy_animal:
                            success = self.on_buy_animal(animal)
                            if success:
                                self.show_message(f"Bought {animal.name}!", GREEN)
                            else:
                                self.show_message("Not enough money!", RED)
                        return None
    ...
    def draw(self, screen: pygame.Surface):
        ...
        if self.current_tab == "buy":
            ...
            animals_color = (100, 140, 100) if self.current_category == "animals" else (70, 70, 70)
            ...
            pygame.draw.rect(screen, animals_color, self.animals_cat_rect, border_radius=5)
            pygame.draw.rect(screen, (120, 160, 120) if self.current_category == "animals" else (90, 90, 90), 
                            self.animals_cat_rect, 2, border_radius=5)
            animals_text = self.small_font.render("Animals", True, WHITE)
            animals_text_rect = animals_text.get_rect(center=self.animals_cat_rect.center)
            screen.blit(animals_text, animals_text_rect)
    ...
    def _draw_buy_tab(self, screen: pygame.Surface):
        ...
        if self.current_category == "seeds":
            ...
            self._draw_animals_category(screen, header_y, player_level)
    ...
    def _draw_animals_category(self, screen: pygame.Surface, header_y: int, player_level: int):
        """Draw animals category items"""
        # Draw animals for sale
        item_y = header_y + 20
        item_height = 70
        item_spacing = 10
        
        for i, animal in enumerate(self.animal_items):
            is_locked = player_level < animal.unlock_level
            ...
            # Animal icon (use Item class to get icon)
            temp_item = Item(animal.item_type, 1)
            icon = temp_item.icon
            icon_rect = icon.get_rect(center=(item_rect.left + 30, item_rect.centery))
            ...
            # Animal name (gray if locked)
            name_color = (128, 128, 128) if is_locked else WHITE
            name_surface = self.item_font.render(animal.name, True, name_color)
            screen.blit(name_surface, (item_rect.left + 60, item_rect.top + 8))
            
            # Animal description or lock message
            if is_locked:
                lock_text = f"Unlocks at level {animal.unlock_level}"
                desc_surface = self.small_font.render(lock_text, True, (255, 150, 150))
            else:
                desc_surface = self.small_font.render(animal.description, True, (180, 180, 180))
            screen.blit(desc_surface, (item_rect.left + 60, item_rect.top + 28))
            
            # Hint text
            hint_text = "Spawns randomly on your farm"
            hint_surface = self.small_font.render(hint_text, True, (150, 200, 150))
            screen.blit(hint_surface, (item_rect.left + 60, item_rect.top + 45))
            
            # Price
            price_text = f"{animal.price} coins"
            price_surface = self.item_font.render(price_text, True, YELLOW)
            price_rect = price_surface.get_rect(right=item_rect.right - 90, centery=item_rect.centery)
            screen.blit(price_surface, price_rect)
            ...
            # Buy button (disabled if locked or not enough money)
            buy_button_rect = pygame.Rect(item_rect.right - 80, item_rect.top + 17, 70, 36)
            can_afford = self.player and self.player.money >= animal.price and not is_locked
            ...
            # Store button rect for click detection (only if not locked)
            if not is_locked:
                self.animal_buttons.append((buy_button_rect, animal))
